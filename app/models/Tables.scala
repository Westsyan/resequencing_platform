...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = Array(Bsa.schema, Project.schema, Sample.schema, Species.schema, Task.schema, User.schema).reduceLeft(_ ++ _)
  @deprecated("Use .schema instead of .ddl", "3.0")
  def ddl = schema

  /** Entity class storing rows of table Bsa
   *  @param id Database column id SqlType(INT), AutoInc
   *  @param bsaname Database column bsaname SqlType(VARCHAR), Length(255,true)
  ...
  final case class BsaRow(id: Int, bsaname: String, accountid: Int, projectid: Int, createdata: DateTime, state: Int)
  /** GetResult implicit for fetching BsaRow objects using plain SQL queries */
  implicit def GetResultBsaRow(implicit e0: GR[Int], e1: GR[String], e2: GR[DateTime]): GR[BsaRow] = GR{
    prs => import prs._
    BsaRow.tupled((<<[Int], <<[String], <<[Int], <<[Int], <<[DateTime], <<[Int]))
  }
  /** Table description of table bsa. Objects of this class serve as prototypes for rows in queries. */
  class Bsa(_tableTag: Tag) extends profile.api.Table[BsaRow](_tableTag, Some("resequencing_platform"), "bsa") {
    def * = (id, bsaname, accountid, projectid, createdata, state) <> (BsaRow.tupled, BsaRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(bsaname), Rep.Some(accountid), Rep.Some(projectid), Rep.Some(createdata), Rep.Some(state)).shaped.<>({r=>import r._; _1.map(_=> BsaRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))

    /** Database column id SqlType(INT), AutoInc */
    val id: Rep[Int] = column[Int]("id", O.AutoInc)
    /** Database column bsaname SqlType(VARCHAR), Length(255,true) */
    val bsaname: Rep[String] = column[String]("bsaname", O.Length(255,varying=true))
    ...
    /** Primary key of Bsa (database name bsa_PK) */
    val pk = primaryKey("bsa_PK", (id, accountid))
  }
  /** Collection-like TableQuery object for table Bsa */
  lazy val Bsa = new TableQuery(tag => new Bsa(tag))
  ...
  class Project(_tableTag: Tag) extends profile.api.Table[ProjectRow](_tableTag, Some("resequencing_platform"), "project") {
  ...
  class Sample(_tableTag: Tag) extends profile.api.Table[SampleRow](_tableTag, Some("resequencing_platform"), "sample") {
  ...
  class Species(_tableTag: Tag) extends profile.api.Table[SpeciesRow](_tableTag, Some("resequencing_platform"), "species") {
  ...
  class Task(_tableTag: Tag) extends profile.api.Table[TaskRow](_tableTag, Some("resequencing_platform"), "task") {
  ...
  class User(_tableTag: Tag) extends profile.api.Table[UserRow](_tableTag, Some("resequencing_platform"), "user") {

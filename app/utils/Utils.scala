...
object Utils {
  ...
  def bsaPath(userId:Int,proId:Int, bsaId:Int) : String ={
    val out = path + "/" + userId + "/" + proId + "/bsa/" + bsaId
    out
  }


  val windowsPath = "F:/platform/resequencing_platform"
  val linuxPath = "/mnt/sdb/platform/resequencing_platform"
  val path = {
  ...
  val samtools = {
    if (new File(windowsPath).exists()) toolPath+"/samtools-0.1.19/samtools" else "samtools"
  }
}
